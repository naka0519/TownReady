...
目的:
- サンプル/初期入力/KB を「横浜市」前提に整備し、実運用に近い具体例で一式生成・安全レビューを検証する。

実装タスク:
- KB: `kb/yokohama_guideline.txt` を作成し、横浜市における避難・運営の要点（地震/風水害/津波・要配慮者配慮・多言語・拠点運営）を収録。Discovery Engine データストア（`kb_default`）に取り込み。
- 既定入力: Web フォームと API デモの初期値を「横浜市戸塚区戸塚町上倉田町７６９−１ / lat=35.398961 / lng=139.537466」に更新。
- サンプル/スクリプト: README のリクエスト例、`docs/*` のサンプル、`scripts/*.sh`（contract/e2e）を同上の住所・緯度経度に更新。
- 参照: `kb_refs` に `kb://yokohama_guideline` を含める例を維持（既存）。

検証プラン:
- Contract Test: `scripts/contract_test.sh` を実行し、Location の既定値更新後も JSON Schema を満たすことを確認。
- E2E スモーク: `scripts/e2e_smoke.sh` を実行し、queued→done、`assets.*_url` の 200 応答を確認。
- KB 検索: `docs/kb-setup.md` の手順で `kb/` を取り込み後、「横浜市 避難」「地域防災拠点」等のクエリで 1 件以上ヒットすることを確認。
- Safety レビュー: 生成後の Safety 出力に `kb://yokohama_guideline` 由来の根拠（タイトル/スニペット）が含まれること（最小）を確認。
- UI 目視: Web フォームの既定値が「横浜市戸塚区戸塚町上倉田町７６９−１ / 35.398961 / 139.537466」になっていることを確認。

ロールバック観点:
- 住所/座標は `.env` ではなくコード・ドキュメントの既定値のみ変更。運用中案件には影響しない。
- KB ファイルは追加のみ。取り込み設定（GCS コネクタ）は従来の `kb/` プレフィックスで共存可能。
